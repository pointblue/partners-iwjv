//document.write("<script src='OpenLayersTileStitchPrinter.js' type='text/javascript'></script>");
var pixel_geom = [];
                
function notice(message)
{       
   // create message with text suppled by call 
     	  jNotify(
		message,
		{
...
{      
    var layername =  encodeURIComponent(ActiveLayer);
    ...
    if(layername.indexOf("Flood") == -1 ) 
    {
       alert("Reports are only available for the Flood layers at this time");
...
function BoxDoneHandler(geom) {

             layername = (ActiveLayer);    //readPseudoCookie('activelayer');           //       
             //make sure that user has chosen a USGS raster layer, cannot yet download the USD+GS vector layers
            var pattern=/Depth|Height|Velocity/i;
            if(!pattern.test(layername)) 
            {
               alert(" At this time you can only download data for flood depth, wave height, and current velocity \nPlease choose one of these layers for download");
                return ;
            }
         layername = urlencode(layername);    
         currentExtent = geom.getBounds();

        var projectedExtent = currentExtent.transform(new OpenLayers.Projection("EPSG:900913"),new OpenLayers.Projection("EPSG:4326"));
        var left = projectedExtent.left;
        var right = projectedExtent.right;
        var upper = projectedExtent.top;
        var lower = projectedExtent.bottom;

      //  alert ('left ' +left+' lower '+lower+' right '+right+' upper '+upper);

    var nodeText = "http://data.calcommons.org/ocofmap/zip.php?activelayer="+layername+"&left="+left+"&right="+right+"&upper="+upper+"&lower="+lower+"";                                   
 
    homer.download(true, nodeText);
   ...
   // now variables              
		    ...
		    displayInLayerSwitcher: false
              ...
              // define the drawFeatureOptions  to handle the vector drawing
               ...
               {       
                ...
                var boxDrawFeatureOptions = 
                {
                    handlerOptions: {sides:4, irregular:true, persist:true, featureAdded: noteAdded}, 
                    'displayClass': 'move_feature_on',      //'olControlDrawFeatureBox',
                    title: "Click and drag to draw a box, and then a link will be provided to export the data",
                    callbacks : {"done": BoxDoneHandler}
                };
            ...
            var boxControl =  new OpenLayers.Control.DrawFeature(boxLayer,
                              OpenLayers.Handler.RegularPolygon, boxDrawFeatureOptions);     
